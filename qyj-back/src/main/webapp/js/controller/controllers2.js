 ...
 * 产品编辑弹窗控制器
 */
qyjBackApp.controller('productEditCtrl', function ($scope, $uibModalInstance, productService, editParams, winParams) {
	// 新增、编辑产品数据
	$scope.edit = {};
	// 弹窗参数
	$scope.winParams = {};
	...
	$scope.uploadImg = function(files) {
		...
		var imageParam = {guid : guid, image : files[0]};
		productService.uploadImage(data).then(function(response) {
		});
//		$http({
//			method : 'post',
...
//		}, function() {})
	...
	// 图片选择完毕后的提交，这个提交并没有提交前面的图片数据，只是提交用户操作完毕后，到底要上传哪些，通过提交键名或者链接，后台来判断最终用户的选择,整个思路也是如此
